...
putexcel set ///
	"$output/HIV testing/childadol/Test uptake incl 0 - child with non-pos mother - 1yr sample ($S_DATE).xlsx", ///
	replace
...
*Exclude zambia because it did not have enough sample
forvalues i=1/5 {
	local k=`i'+1
	preserve
...
putexcel set ///
	"$output/HIV testing/childadol/Test uptake incl 0 - child with pos mother - 1yr sample ($S_DATE).xlsx", ///
	replace
...
forvalues i=1/5 {
	local k=`i'+1
	preserve
	keep if country2==`i' & status_enrol_index==1 & country2 == 5
	putexcel A`k' = "Country `i'"
	count if total_tests_cat!=.
	local total=`r(N)'
	count if total_tests_cat==0
...
* --> Number of tests for children of non-pos mothers (including 0)
putexcel set ///
	"$output/HIV testing/childadol/Test uptake incl 0 - child with non-pos mother - 2yr sample ($S_DATE).xlsx", ///
	replace
...
* --> Number of tests for children of pos mothers (including 0)
putexcel set ///
	"$output/HIV testing/childadol/Test uptake incl 0 - child with pos mother - 2yr sample ($S_DATE).xlsx", ///
	replace
