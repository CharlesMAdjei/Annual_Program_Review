...
*** Basic variable creation for partners
* 28 April 2021



*------------------------------------------------------------------------------*
* Create partner dataset

* --> Open hh member registration dataset and merge with partner
use "$temp/reg_hhmem.dta", clear
drop if dreg_hhmem_timeend_date<td(01jan2020) | ///
	dreg_hhmem_timeend_date>td(30jun2021)

merge 1:m hhmem_id using "$temp/partner.dta"
drop if dpartner_timeend_date<$beg_period1 | ///
	(dpartner_timeend_date>$end_period & dpartner_timeend_date<.)
drop if _merge==2
	// Drop those only in the partner dataset.

keep if inlist(dreg_hhmem_hhmem_type, "male_partner")
	
	
* --> Fill in missing country and facility in reg form with partner form
foreach i in country facility {
	gen `i'=dreg_hhmem_`i'
	replace `i'=dpartner_`i' if `i'=="" 
	count if `i'==""
}
...
sort hhmem_id dpartner_timeend_date
...
drop if enrolment_date<td(01jan2021) & keep_2y==0 & keep_1y==1
drop if enrolment_date>td(30jun2021)
tab country if id_tag==1
	// All those enrolled before 2018 are removed. This is just the Lesotho
	// sites that started using the app in 2017.
	// Sample sizes:
		// Kenya: 473
		// Malawi: 3437
		// Zambia: 1197

*replace keep_1y=0 if enrolment_date<td(01jan2020)
	// Must exclude clients that enrolled before 1 Jan 2019 for the 1
	// year analysis so the samples are equivalent - i.e. all enrolled between
	// 1 Jan and 30 Jun 2019.
	
tab1 keep* if id_tag==1, m	
	// 2840 in the 2 year sample, 2885 in the 1 year sample
...
* --> DOB 
foreach x in reg_hhmem_dob partner_dob {
	cap drop `x'
...
/*
count if reg_hhmem_dob!=partner_dob & partner_dob!=. & reg_hhmem_dob!=.
br reg_hhmem_dob partner_dob ///
	if reg_hhmem_dob!=partner_dob & partner_dob!=. & reg_hhmem_dob!=.
*/

gen dob=reg_hhmem_dob
replace dob=partner_dob if dob==.
format %d dob
...
count if age_enrol<16 & id_tag==1
count if age_enrol>80 & age_enrol<. & id_tag==1
	// Assuming youngest in partner form could be 16. This is still probably
	// too low but who knows.
	
replace age_enrol=. if age_enrol<16 | (age_enrol>80 & age_enrol<.)
replace dob=. if age_enrol<16
count if age==. & id_tag==1
gen age_missing=age==.
tab age_miss if id_tag==1
	// 2% missing age at enrolment
...
* --> HIV status in partner form
gen status_partner=1 if strpos(dpartner_hiv_status, "positive")
replace status_partner=0 if strpos(dpartner_hiv_status, "negative")
replace status_partner=2 if strpos(dpartner_hiv_status, "unknown")
label val status_partner status
label var status_partner "HIV status in partner form"
tab status_partner

gen status_partner1x=status_partner if obs==1
egen status_partner_first=max(status_partner1x), by(hhmem_id)
drop status_partner1x
label var status_partner_first "HIV status in first partner form"
label val status_partner_first status
tab status_partner_first if id_tag==1, m


* --> Compare HIV status in reg index and partner on first submission	
gen same_dayx=enrolment_date==dpartner_timeend_date 
egen same_day=max(same_dayx), by(hhmem_id)
drop same_dayx
label var same_day "1st partner form submitted on same day as reg index form"
count if status_enrol!=status_partner_first & same_day==1
/*
sort hhmem_id obs
br hhmem_id obs status_enrol status_partner_first if ///
	status_enrol!=status_partner_first & ///
	same_day==1 & status_enrol!=. & status_partner_first!=.
*/

replace status_enrol=status_partner_first if same_day==1 & ///
	status_partner_first!=2
	// Give priority to status in partner form if it is different to
	// status in reg index form and submitted on the same day as the reg form.

replace status_enrol=status_partner_first if status_enrol==.
tab status_enrol if id_tag==1, m

tab country if id_tag==1 & status_enrol!=1
	// Sample sizes of non-positive partners:
		// Kenya: 382
		// Malawi: 3076
		// Zambia: 533
...
gen status_partner_lastx=status_partner if obs==obs_max
egen status_partner_last=max(status_partner_lastx), by(hhmem_id)
label var status_partner_last "HIV status in last partner form"
label val status_partner_last status
count if status_enrol==1 & status_partner_last!=1
tab status_enrol status_partner_last, m
/*
sort facility hhmem_id obs
br hhmem_id obs status_enrol status_partner if ///
	status_enrol!=status_partner_last
*/
	// Seems OK

count if status_enrol!=status_partner_last & status_enrol==0
tab status_partner_last if ///
	status_enrol!=status_partner_last & status_enrol==0 & id_tag==1, m
	// Of those who started out negative and have a different status in
	// their latest partner form, 38 are positive and 415 have no status. 
...
/*
Visit date is based on the completion time of the partner form. 
*/

* --> Visit date (based on partner timeend date)
gen visit_date=dpartner_timeend_date
format %d visit_date
label var visit_date "Visit date (partner form sub date)"


* --> First and last visit dates
egen visit_date_first=min(visit_date), by(hhmem_id)
format %d visit_date_first
label var visit_date_first "Date of first home visit (1st partner form)"
egen visit_date_last=max(visit_date), by(hhmem_id)
	label var visit_date_last "Date of last home visit (last partner form)"
format %d visit_date_last
tab1 visit_date_*st, m
	// Those without a first or last visit are those who never had a partner
	// form submitted or whose partner form was somehow missing the form sub
	// date which happens sometimes in app2. No idea why.
...
* Number of visits 
/*
This is a count of form submissions in the partner form which actually records
services delivered. Those without any partner forms are assigned a value of 
0 for visits. 
*/
...
As above, if the first partner form is submitted at the same time as the
...
keeporder country hhmem_caseid obs visit_date_last visit_date gap_days ///
	enrolment_date status_enrol keep* age_enrol
...
label var gap_days_lastv_end_cat ///
	"Gap between lst visit and 31 Dec 2018 in days" 

	
* --> Save data for output
gen db="partner"
save "$temp/partner - gap betw visits - both samples.dta", replace
...
gen hivtest_date=date(dpartner_hiv_retest_done_date, "YMD") 
replace hivtest_date=date(dpartner_hiv_retest_done_date, "DMY") if ///
	hivtest_date==.
format %d hivtest_date
label var hivtest_date "HIV test date"

cap drop hcttest_date
ren dpartner_hct_date dpartner_hct_done_date
	// Didn't name variables in partner form consistently with child adol form
	
gen hcttest_date=date(dpartner_hct_done_date, "YMD") 
replace hcttest_date=date(dpartner_hct_done_date, "DMY") if ///
	hcttest_date==.
...
tab hivtest_befenrol
	// 45% of recorded HIV test dates are before enrolment

	
* --> Result
gen hivtest_result=dpartner_hiv_result=="tested_positive" if ///
	dpartner_hiv_result!=""
label val hivtest_result status
label var hivtest_result "HIV test result"

gen hcttest_result=dpartner_hct_result=="tested_positive" if ///
	dpartner_hct_result!=""
...
* --> ART start date
foreach x in reg_hhmem_init_date partner_init_date {
	cap drop `x'
	gen `x'x=date(d`x', "YMD") 
	replace `x'x=date(d`x', "DMY") if `x'==.
	egen `x'=max(`x'x), by(hhmem_id)
...
replace artstart_date=partner_init_date if artstart_date==.
format %d artstart_date
label var artstart_date "Date of ART initiation"
count if status_final!=1 & artstart_date!=.
	// 1 person who is non-pos at the end of the period has an ART start date.
	
count if status_final==1 & artstart_date==. & id_tag==1
	// 75 positive partners have no ART start date
...
* --> ART refill dates
tab1 dpartner_artrefill_done dpartner_artrefill_latest ///
	dpartner_artrefill_latest_date if status_final==1
foreach x in partner_artrefill_latest_date {
	cap drop `x'
	gen `x'=date(d`x', "YMD") 
	replace `x'=date(d`x', "DMY") if `x'==.
	format %d `x'
}
egen latest_refill=max(partner_artrefill_latest_date), by(hhmem_id)
replace artstart=1 if latest_refill!=. 

tab status_final if id_tag==1
tab partner_artrefill_latest_date
	// No partner ART refill dates at all
...
* --> Make new variables
gen vl_test_done=dpartner_vl_test_done=="yes" if dpartner_vl_test_done!=""
label var vl_test_done "VL test done"
label val vl_test_done yes_no
ren dpartner_vl_result vl_result
label var vl_result "VL result"
ren dpartner_vl_result_number vl_result_num
label var vl_result_num "VL result number"
destring vl_result_num, replace
foreach x in vl_test_due_date {
	gen `x'=date(dpartner_`x', "YMD") 
	replace `x'=date(dpartner_`x', "DMY") if `x'==.
	format %d `x'
}
drop dpartner_vl*
/*
...
tab vl_test_ever if status_final!=1 & id_tag==1
	// No one who is not marked as positive as their final status has anu
...
foreach x in adh_remember adh_stop_better adh_7_forget adh_stop_worse ///
	adh_notadherent {
		cap drop `x'
		gen `x'=(dpartner_`x'=="1" | dpartner_`x'=="yes") if ///
			dpartner_`x'!=""
...
gen adh_7_forget_num=dpartner_adh_7_forget_num
...
replace perc_visits_adh_measure=1 if num_adh_measures>=1 & ///
	...
	// because their partner form submission date is blank. No idea why that
...
* --> Save 
save "$temp/partner - full dataset - both samples.dta", replace
forvalues i=1/2 {
	preserve
	keep if keep_`i'y==1
	save "$temp/partner - full dataset - `i'yr sample.dta", replace
	restore
}

forvalues i=1/2 {
	use "$temp/partner - gap betw visits - both samples.dta", clear
	keep if keep_`i'y==1
	save "$temp/partner - gap betw visits - `i'yr sample.dta", replace
}
