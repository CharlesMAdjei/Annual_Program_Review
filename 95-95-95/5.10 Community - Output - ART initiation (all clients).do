...
forvalues i=1/7 {	
	tabout client1 artstart if country2==`i' ///
		using "$output/ART initiation/ART initiation - All - 1 year sample country `i' ($S_DATE).xls", ///
		c(row) f(1p 0c) lay(row) npos(row) nlab(Sample size) ///
		clab(_ _) per h3(nil) ///
		replace 
}
tabout client1 artstart ///
	using "$output/ART initiation/ART initiation - All - 1 year sample ($S_DATE).xls", ///
	c(row) f(1p 0c) lay(row) npos(row) nlab(Sample size) ///
	clab(_ _) per h3(nil) ///
	replace 
...
forvalues i=1/5 {	
	tabout client1 artstart if country2==`i' ///
		using "$output/ART initiation/ART initiation - All - 2 year sample country `i' ($S_DATE).xls", ///
		c(row) f(1p 0c) lay(row) npos(row) nlab(Sample size) ///
		clab(_ _) per h3(nil) ///
		replace 
}
tabout client1 artstart ///
	using "$output/ART initiation/ART initiation - All - 2 year sample ($S_DATE).xls", ///
	c(row) f(1p 0c) lay(row) npos(row) nlab(Sample size) ///
	clab(_ _) per h3(nil) ///
	replace 
...
forvalues i=1/3 {	
	tabout client1 artstart if country2==`i' ///
		using "$output/ART initiation/ART initiation - All - 3 year sample country `i' ($S_DATE).xls", ///
		c(row) f(1p 0c) lay(row) npos(row) nlab(Sample size) ///
		clab(_ _) per h3(nil) ///
		replace 
}
tabout client1 artstart ///
	using "$output/ART initiation/ART initiation - All - 3 year sample ($S_DATE).xls", ///
	c(row) f(1p 0c) lay(row) npos(row) nlab(Sample size) ///
	clab(_ _) per h3(nil) ///
	replace 
