...
duplicates drop
save "$ric_temp/ric sample facilities - 3 year cohort.dta", replace
export excel using "$output/Checks/Facilities in RIC sample - 3 year cohort ($S_DATE).xlsx", ///
...
sort country facility
duplicates drop
save "$ric_temp/ric sample facilities - 2 year cohort.dta", replace
export excel using "$output/Checks/Facilities in RIC sample - 2 year cohort ($S_DATE).xlsx", ///
	firstrow(variables) replace
restore
...
foreach x in Ghana {
	...
	sort facility
	export excel using ///
		"$ric_output/Checks/APR 2021 - RIC - Eligible sites 3 year cohort ($S_DATE).xlsx", ///
...
foreach x in Ghana {
	...
	sort facility
	export excel using ///
		"$ric_output/Checks/APR 2021 - RIC - Eligible sites 2 year cohort ($S_DATE).xlsx", ///
		firstrow(varlabels) sheet("`x'") replace
	restore
}
foreach x in Kenya Lesotho Malawi SA Uganda Zambia {
